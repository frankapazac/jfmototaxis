...
public class UTFEncodingUtil
{
  ...
  public static void decodeObjectUTF(Object object) {
	  try {
		  Field[] fields=object.getClass().getDeclaredFields();
		  int cont=1;
		  for(Field f:fields){
			  //System.out.println(f.getName());//Hasta aca obtenemos los nombres de los atributos del objeto
			  String field=f.getName();
			  	try{
					Method getter = object.getClass().getMethod("get" +
					          String.valueOf(field.charAt(0)).toUpperCase() +
					          field.substring(1));
					Object value = getter.invoke(object, new Object[0]);
					//System.out.println("Valor: "+value);
					Method setter = object.getClass().getMethod("set" +
					          String.valueOf(field.charAt(0)).toUpperCase() +
					          field.substring(1),fields[cont].getType());
					cont++;
					setter.invoke(object, new Object[]{decodeUTF(value.toString())});
					Object values = getter.invoke(object, new Object[0]);
					//System.out.println("Nuevo: "+values);
			  	}catch(Exception ex){
			  	}
		  }
	  } catch (Exception e) {
